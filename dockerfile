...
#1.1 Tạo symlink libaio.so.1 nếu hệ thống chỉ có libaio.so.1t64
...
# 3. Cài ChromeDriver ổn định
RUN LATEST_DRIVER_URL=$(curl -s https://googlechromelabs.github.io/chrome-for-testing/last-known-good-versions-with-downloads.json | \
      jq -r '.channels.Stable.downloads.chromedriver[] | select(.platform=="linux64") | .url') && \
    mkdir -p /tmp/chromedriver && cd /tmp/chromedriver && \
    wget "$LATEST_DRIVER_URL" -O chromedriver.zip && unzip chromedriver.zip && \
...
# 9. Thiết lập thick mode cho Oracle trong virtualenv
ENV ORACLE_INSTANTCLIENT=/opt/oracle/instantclient
